...
- Backend only (REST API), frontend robi inna osoba
- Security: Spring Security + JWT (stateless, bearer token w headerze Authorization)
- Login/hasło w bazie, BCrypt, 4 role: ADMIN, PLANNER, SUPERVISOR, PILOT
- Rejestracja użytkownika: endpoint POST /api/auth/register
- Deploy: Render.com przez GitHub, PostgreSQL na Render (zdalny dostęp)
- Java 21, Spring Boot 4.0.5, Lombok, JPA, springdoc-openapi (Swagger)
- Wywalić z pom.xml: spring-boot-starter-security-oauth2-client, spring-boot-starter-mail (niepotrzebne)
- Dodać do pom.xml: jjwt-api, jjwt-impl, jjwt-jackson (0.12.6)
- Struktura pakietów: config, model/entity, model/dto, model/enums, repository, service, controller, security

## 12. Plan implementacji

### Faza 1 — fundament (na start)
1. Wywalić zbędne zależności z pom.xml, dodać JJWT
2. Encje JPA + enumy (wszystkie 6 tabel + statusy + role)
3. Security: JwtUtil, JwtAuthFilter, SecurityConfig
4. AuthController: POST /api/auth/login + POST /api/auth/register
5. User CRUD + dane testowe (seed po 1 userze na rolę)

### Faza 2 — hackathon (moduł po module)
6. Helikoptery — CRUD
7. Członkowie załogi — CRUD + walidacja email/licencji
8. Lądowiska — CRUD
9. Planowane operacje lotnicze — CRUD + statusy + uprawnienia per rola + KML
10. Zlecenia na lot — CRUD + walidacje (przegląd, licencja, szkolenie, waga, zasięg) + zmiany statusów

### Faza 3 — bonus (jeśli starczy czasu)
11. Mapa — wyświetlanie punktów z KML + lądowisk
12. Historia zmian — audit trail na operacjach
13. Filtrowanie i sortowanie na listach